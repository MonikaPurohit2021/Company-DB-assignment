...
essn = Employee.query.with_entities(Employee.ssn).all()
pno = Project.query.with_entities(Project.pnumber).all()
...
for row in essn:
    rowDict=row._asdict()
    results.append(rowDict)
essnChoices = [(row['ssn'],row['ssn']) for row in results]

results=list()
for row in pno:
    rowDict=row._asdict()
    results.append(rowDict)
pnoChoices = [(row['pnumber'],row['pnumber']) for row in results]
...
class AssignUpdateForm(FlaskForm):
    essn = SelectField("Select Employee", choices=essnChoices)  # myChoices defined at top
    pno = SelectField("Select Project", choices=pnoChoices)  # myChoices defined at top
    hours=StringField('Hours', validators=[DataRequired()])
    submit = SubmitField('Update this Project Assingment')
...
class AssignForm(AssignUpdateForm):
    ...
    pno = SelectField("Select Project", choices=pnoChoices)  # myChoices defined at top
    hours=StringField('Hours', validators=[DataRequired()])
    submit = SubmitField('Add New Assingment')
